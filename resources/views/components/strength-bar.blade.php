...
<div class="w-full">
    ...
    <div class="relative mt-1 h-4 text-[10px] text-faint">
        @foreach($levels as $i => $lvl)
            ...
            <span class="absolute -translate-x-1/2 {{ $i === $eval['level_index'] ? 'text-ink font-semibold' : '' }}" style="left: {{ $centre }}%;">{{ $lvl }}</span>
        @endforeach
    </div>
    ...
    @if(count($pops) > 1)
        <div class="mt-1 flex flex-wrap gap-2 text-[11px]">
            @if(isset($pops['gym']))
                <span class="rounded-sm bg-info-soft text-info px-2 py-0.5">Gym: {{ $fmt($pops['gym']['percentile']) }}%@if(!empty($pops['gym']['sample'])) <span class="text-info">(n={{ number_format($pops['gym']['sample']) }})</span>@endif</span>
            @endif
            @if(isset($pops['verified']))
                <span class="rounded-sm bg-accent-soft text-accent px-2 py-0.5">Verified/competition: {{ $fmt($pops['verified']['percentile']) }}%@if(!empty($pops['verified']['sample'])) <span class="opacity-75">(n={{ number_format($pops['verified']['sample']) }})</span>@endif</span>
            @endif
        </div>
    @endif
